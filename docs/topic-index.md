...
| `runtime log` / `运行日志` / `回放` | [Observability / Tracing：智能体可观测性](observability-tracing-agent-workflows.md)、[多语言 Trace Context 传播样例](../examples/trace-observability/trace-context-multilang.md) | 第 8-10、12 章 | [多语言运行日志回放样例](../examples/trace-observability/trace-runtime-log-replay.md)、[Agent Trace 可观测性样例](../examples/trace-observability/index.md) | 运行日志适合复盘和截图，不适合替代事实事件或原始 trace |
