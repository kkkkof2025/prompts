...
| `observability` / `tracing` / `trace` | [Observability / Tracing：智能体可观测性](observability-tracing-agent-workflows.md)、[Durable Execution：持久化执行与 agent 长任务](durable-execution-agent-workflows.md) | 第 8-10、12 章 | [Agent Trace 可观测性样例](../examples/trace-observability/index.md)、[OpenClaw 多 agent 联动教程](openclaw-multi-agent-linkage.md)、[OpenClaw、Node.js 与超级大脑架构](openclaw-superbrain-architecture.md) | Trace 不是事实源，不能替代 Event Sourcing；同时要做脱敏和留存策略 |
